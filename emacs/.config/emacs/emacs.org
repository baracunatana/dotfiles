...
** Introducción
Para correo electrónico uso mu4e. Este paquete hace cosas no-estandar y por eso no se pude usar straight ni use-package para su instalación. Toca a la antigüita.

** Instalación
*** Linux
La instalación requiere un =sudo apt-get install mu4e= en ubuntu 20.04.

En NixOS toca instalar los paquetes =mu= y =isync=.

*** TODO Configuración de mbsync
=mbsync= es la aplicación que descarga el correo, y =mu= es la aplicación que lo indexa. 
