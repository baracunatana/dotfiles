...
Luego se cambia el /hostname/ de Ubuntu a Kanon y se compila emacs con /pgtk/, /native compilation/, y /json/.
#+bigin_src sh :tangle no
hostnamectl set-hostname kanon
sudo apt update
sudo apt full-upgrade

git clone git://git.sv.gnu.org/emacs.git
sudo apt install build-essential libgtk-3-dev libgnutls28-dev libtiff5-dev libgif-dev libjpeg-dev libpng-dev libxpm-dev libncurses-dev texinfo libgccjit0 libgccjit-10-dev gcc-10 g++-10 libjansson4 libjansson-dev
...
*** Script de inicialización en Ubuntu
:PROPERTIES:
:header-args:sh: :tangle (if (equal (system-name) "kanon") "kanon-init.sh" "no") :tangle-mode (identity #o755) :shebang #!/bin/bash
:END:

Luego de tener una instalación de emacs estable, se clona este repositorio y se hace un /tangle/ a este archivo. Luego se ejecuta ~./kanon-init.sh~
...
**** Compilar emacs desde repositorio con /pgtk/, /native compilation/, y /json/ 
