...
#+begin_src nix :tangle no :noweb-ref nixos-imports
...
***** Extensiones
Para activar las extensiones de =gnome=, se debe activar el =gnome-settings-deamon= con
#+begin_src nix :tangle no :noweb-ref nixos-base
services.udev.packages = with pkgs; [ gnome.gnome-settings-daemon ];
#+end_src

Ahora, se pueden instalar los paquetes de las extensiones que se quieren tener activadas:
...
** home-manager
Para configurar y usar =home-manager=, lo primero que se debe hacer es importar el canal. Esto se puede hacer con =# $ nix-channel --add https://github.com/nix-community/home-manager/archive/release-22.05.tar.gz home-manager=, pero es mejor tenerlo todo en =configuration.nix=.
...
=spotifyd= es un servicio, así que se configura con opción en lugar de con instalación de paquete. Si se hace con instalación de paquete, se tendría que corre siempre que se quiera usar. En cambio, con opción de servicio se arranca por defecto con el sistema. Para que esto funcione correctamente, tiene que configurarse con [[home-manager]].

#+begin_src nix :tangle no :noweb-ref home-manager-juan :noweb no-export
services.spotifyd = {
  enable = true;
  settings = {
    global = {
      username = "<<secreto("spotify.com", "user")>>";
      password_cmd = "gpg2 -q --for-your-eyes-only --no-tty -d /home/juan/.config/spotifyd/.spotifyd-pass.gpg";
      use_mpris = true;
      backend = "alsa";
      device = "default" ;
...
#+begin_src nix :tangle no :noweb-ref nixos-let
  emacsWithPackages = (pkgs.emacsPackagesFor pkgs.emacsPgtkNativeComp).emacsWithPackages (epkgs: ([epkgs.vterm epkgs.pdf-tools]));
#+end_src
...
#+begin_src emacs-lisp :noweb-ref org-config :tangle no
   (with-eval-after-load 'org-agenda 
     (add-to-list 'org-agenda-prefix-format '(agenda . " %i %?-12t% s %b"))
     (add-to-list 'org-agenda-prefix-format '(tags . " %i %b"))
     (add-to-list 'org-agenda-prefix-format '(todo . " %i %b")))
#+end_src
...
#+begin_src emacs-lisp :noweb-ref org-general :tangle no
  (j/lider-local
    ...
    "l" '(org-insert-link :which-key "insertar enlace")
    "g" '(org-store-link :which-key "guardar enlace")
...
#+begin_src emacs-lisp :noweb-ref mu4e-general :tangle no
  ...
  (j/lider-local 
    ...
    "lg" '(org-store-link :which-key "guardar enlace desde org")
...
****** Agregar archivos con tag =proyecto=, =area=, e =identidad= a la vista de agenda
Se crea una función para agregar todos los archivos de =org-roam= que tengan tag =proyecto=, =area=, o =identidad=, y luego se aconseja a =org-agenda= para que corra esa función antes de generar la agenda. Con esto se logra que la agenda se calcula incluyendo todas las tareas de archivos =org-roam= con tag =proyecto=. 

#+begin_src emacs-lisp :noweb-ref org-roam-config :tangle no
  (defun j/poblar-archivos-agenda ()
      (interactive)
      (mapcar (lambda (arch) (add-to-list 'org-agenda-files (car arch)))
              (org-roam-db-query [ :select nodes:file
                                   :from tags
                                   :inner :join nodes
                                   :on (= tags:node-id nodes:id)
                                   :where (= tags:tag "proyecto")] ","))
      (mapcar (lambda (arch) (add-to-list 'org-agenda-files (car arch)))
              (org-roam-db-query [ :select nodes:file
                                   :from tags
                                   :inner :join nodes
      ...
      (mapcar (lambda (arch) (add-to-list 'org-agenda-files (car arch)))
              (org-roam-db-query [ :select nodes:file
                                   ...
                                   :inner :join nodes
                                   :on (= tags:node-id nodes:id)
                                   :where (= tags:tag "identidad")] ",")))

  (advice-add 'org-agenda
              :before
              #'j/poblar-archivos-agenda)
...
******* Company
Si bien ~company~ se puede usar en muchos contextos, por ahora lo uso únicamente con ~lsp~.
...
******* Python
El primer paso para esta configuración es [[Microsoft Python LSP][instalar el LSP de Microsoft para python]]. Luego de esto, se hacen las configuraciones necesarias en emacs:
...
***** =find-lisp=
=find-lisp= es una librería incluida en emacs (pero no activada por defecto) que permite buscar archivos y carpetas de forma sencilla desde lisp. Se usa para resolver un problema con la exportación de archivos de [[*=org-roam=][=org-roam=]] (ver https://github.com/kaushalmodi/ox-hugo/issues/483).
...
=syncthing= permite sincronizar carpetas entre dispositivos. Funciona de forma similar a =dropbox=, pero con la limitación de que la sincronización sólo ocurre cuando los dispositivos están prendidos de forma simultanea.

La instación de syncthing se hace como si fuera un servicio de [[home-manager]].
...
#+begin_src python :tangle no :noweb-ref qutebrowser-config
  c.aliases = {
      ...
      "wq": "quit --save",
      "wqa": "quit --save",
      "arqEmp": "open -t https://campusvirtuallms.javeriana.edu.co/d2l/home/104079",
      "pensaSis": "open -t https://campusvirtuallms.javeriana.edu.co/d2l/home/104740",
      "sisInfo": "open -t https://campusvirtuallms.javeriana.edu.co/d2l/home/103789",
  }
#+end_src
...
#+end_src
**** Nyxt
Nyxt se instala en *shion*, *shura*, y *kanon*. A continuación se definen los dos bloques de instalación
#+begin_src nix :tangle no :noweb-ref nixos-paquetes
  nyxt
#+end_src
