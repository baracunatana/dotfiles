...
(defcustom j/excorporate-org-save-file
  (concat user-emacs-directory ".exco-calendar.org")
  "Where `j/exco-org-show-day' saves its results.
This file's contents are overwritten on each invocation of
`j/exco-org-show-day'.")
#+end_src
...
#+begin_src emacs-lisp :tangle no :noweb-ref excorporate-config
  ...
  (add-to-list 'org-agenda-files j/excorporate-org-save-file)
  
  (defun j/exco-org-insert-meetings (identifier response)
    "Insert the connection IDENTIFIER's meetings from RESPONSE."
    (with-current-buffer (find-file-noselect j/excorporate-org-save-file)
