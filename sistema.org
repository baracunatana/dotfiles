...
****** Agregar archivos con tag ~proyecto~ a la vista de agenda
Se crea una función para agregar todos los archivos de ~org-roam~ que tengan tag ~proyecto~ y luego se aconseja a ~org-agenda~ para que corra esa función antes de generar la agenda. Con esto se logra que la agenda se calcula incluyendo todas las tareas de archivos ~org-roam~ con tag ~proyecto~. 
